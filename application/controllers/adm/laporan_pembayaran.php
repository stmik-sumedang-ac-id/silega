...
class Laporan_pembayaran extends MY_Controller {
	...
	public function getdata($tahun) {
		...
		foreach($bulans as $index=>$value){
			...
			$data2 = $this->db->query("SELECT (SELECT COUNT(*) from pengajuan where status = '1' AND MONTH(waktu_pengajuan) = $index+1 AND YEAR(waktu_pengajuan) = $tahun) belumbayar,(SELECT COUNT(*) from pengajuan where status = '2' AND MONTH(waktu_pengajuan) = $index+1 AND YEAR(waktu_pengajuan) = $tahun) sudahbayar")->row();
